...
const surveyCard = ({ id, title, description, questionsCount }) => {
	return (
		<Link
			to={`/results/${id}`}
			className='bg-white rounded-xl px-4 shadow-md cursor-default max-h-36 scrollbar-thin scrollbar-thumb-red-300 scrollbar-track-transparent overflow-y-scroll scrollbar-thumb-rounded-full scrollbar-track-rounded-full'
		>
